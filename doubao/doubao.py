...
SHORT_PROMPT = (
    "Provide a concise description of the industrial defect in the image. "
    ...
    "Example response format:\n"
    "Metal surface has a scratch with a linear mark. (70 chars)"
)
...
def generate_short_text(long_text, object_category, defect_category):
    ...
    short_text = f"{object_category} with {defect_category}"
...
def get_available_datasets(input_dir):
    ...
    for item in os.listdir(input_dir):
        if os.path.isdir(os.path.join(input_dir, item)):
            json_path = os.path.join(input_dir, item, "dataset_info_1.json")
...
def process_dataset(dataset, input_dir):
    ...
    json_path = os.path.join(dataset_path, "dataset_info_1.json")

    print(f"\nProcessing dataset: {dataset}")

    # Create directories for saving prompts
    dataset_prompt_dir = os.path.join(dataset_path, "prompt")
    dataset_prompt_short_dir = os.path.join(dataset_path, "prompt_short")
    ...
    for image_info in tqdm(metadata["images"], desc="Processing images"):
        try:
            ...
            # Get short analysis (either from API or generated from long text)
            short_analysis = analyze_with_doubao(image_path, "short")

            if long_analysis and short_analysis:
                # Build paths for saving
                relative_path = os.path.dirname(image_info["image_path"])
                base_name = os.path.splitext(os.path.basename(image_path))[0]

                # Save long analysis
                long_filename = f"{base_name}_analysis.txt"
                long_path = os.path.join(dataset_prompt_dir, relative_path, long_filename)
                ...
                short_path = os.path.join(dataset_prompt_short_dir, relative_path, short_filename)
                os.makedirs(os.path.dirname(short_path), exist_ok=True)
                with open(short_path, 'w') as f:
                    f.write(short_analysis)

                # Process paths
                long_rel_path = os.path.relpath(long_path, dataset_path)  # Relative to dataset path
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Process datasets with Doubao API for image analysis")
    ...
    for dataset in available_datasets:
        dataset_prompt_dir = os.path.join(args.input_dir, dataset, "prompt")
        dataset_prompt_short_dir = os.path.join(args.input_dir, dataset, "prompt_short")
        os.makedirs(dataset_prompt_dir, exist_ok=True)
        os.makedirs(dataset_prompt_short_dir, exist_ok=True)
